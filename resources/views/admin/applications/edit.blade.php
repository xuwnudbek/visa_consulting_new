...
@section('content')
    <main class="h-full pb-16 overflow-y-auto">
        <div class="container my-6 px-6 mx-auto grid">
            ...
            <!-- Form -->
            <form action="{{ route('admin.applications.update', $application->id) }}" method="POST" enctype="multipart/form-data" class="flex flex-col bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md space-y-6">
                @csrf
                @method('PUT')
                        ...
                        <label class="block text-sm">
                            ...
                            <select id="client-select" required name="client_id" class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700
                                ...
                                @foreach ($clients as $client)
                                    <option value="{{ $client['id'] }}" {{ old('client_id', $application->user_id) == $client['id'] ? 'selected' : '' }}>{{ $client['name'] }}</option>
                                @endforeach
                            </select>
                        </label>
                        ...
                        <label class="block text-sm">
                            ...
                            <select name="country_id" required class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700
                                ...
                                @foreach ($countries as $country)
                                    <option value="{{ $country['id'] }}" {{ old('country_id', $application->country_id) == $country['id'] ? 'selected' : '' }}>{{ $country['name'] }}</option>
                                @endforeach
                            </select>
                        </label>
                        ...
                        <label class="block text-sm">
                            ...
                            <select name="field_id" required class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700
                                ...
                                @foreach ($fields as $field)
                                    <option value="{{ $field['id'] }}" {{ old('field_id', $application->field_id) == $field['id'] ? 'selected' : '' }}>{{ $field['title'] }}</option>
                                @endforeach
                            </select>
                        </label>
                    ...
                    <div class="flex flex-col w-full gap-4">
                        ...
                        <label class="block text-sm">
                            ...
                            <textarea name="comment" rows="5" class="block disabled:opacity-50 w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700
                                   focus:border-purple-400 focus:outline-none focus:shadow-outline-purple
                                   dark:text-gray-300 dark:focus:shadow-outline-gray form-input" placeholder="Enter comment">{{ old('comment', $application->comment) }}</textarea>
                            @error('comment')
                                <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                            @enderror
                        </label>
