...
@section('content')
    <main class="h-full pb-16 overflow-y-auto">
        <div class="container my-6 px-6 mx-auto grid">
            ...
            <form action="{{ route('admin.fields.update', $field->id) }}" method="POST" enctype="multipart/form-data" class="w-full bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="flex flex-col items-start justify-center gap-4 sm:flex-row sm:space-x-1 mb-4">
                    <div class="flex flex-col gap-2 w-full">
                        <div class="w-full sm:w-1/2">
                            ...
                            <input type="text" name="title" id="title" class="mt-1 block w-full form-input text-gray-700 dark:text-gray-300 dark:bg-gray-700 rounded-md border-gray-300 dark:border-gray-600 focus:border-purple-500 focus:ring focus:ring-purple-200" value="{{ old('title', $field->title) }}" placeholder="Enter field title">
                            @error('title')
                        ...
                        <div class="w-full sm:w-1/3">
                            ...
                            <input type="number" name="amount" id="amount" class="mt-1 block w-full form-input text-gray-700 dark:text-gray-300 dark:bg-gray-700 rounded-md border-gray-300 dark:border-gray-600 focus:border-purple-500 focus:ring focus:ring-purple-200" value="{{ old('amount', $field->amount) }}" placeholder="Enter amount">
                            @error('amount')
                                <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                            @enderror
                        </div>
